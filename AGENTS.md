...
Workflow after changing `src/backend/db/schema.js`: `db:generate`, then `db:push`.
...
```
src/backend/cms/schema.js    ← THE SOURCE OF TRUTH
...
src/shared/config.js         LANGUAGES (en, fa, ps) + NAV_IDS, shared with the site
```
...
`schema.js`; add the column in `src/backend/db/schema.js`; run `db:generate` +
...
`getContent()` in `src/backend/cms/content.js` is wrapped in `unstable_cache` with the
tag `"content"`. Every admin write calls `bustContent()` (`updateTag("content")` +
`revalidatePath("/")`) in `src/backend/cms/actions.js`, so a save is visible on the
...
## 7. Folder layout — frontend / backend / shared

The code is split into three top-level folders under `src/`. This is a folder
separation inside ONE Next.js app, not two deployables: one `npm run dev`, one
build, one Netlify site.

```
src/
  app/        ROUTES ONLY — Next.js requires them here. Keep these files thin:
              load data, pass it to a component. No business logic.
  frontend/   everything the visitor sees        → import as @frontend/…
  backend/    data, server logic, admin panel    → import as @backend/…
  shared/     the few things both sides need     → import as @shared/…
  proxy.js    middleware (must stay at this exact path)
```

Each folder has its own README with the detail: `src/frontend/README.md`,
`src/backend/README.md`, `src/shared/README.md`. Read the one you are working in.
...
src/frontend/
  components/             Site.jsx + Hero/About/Work/Experience/Skills/
                          Services/Reviews/Contact/Navbar/Footer, theme and
                          language switches, Providers
  components/three/       3D scenes (hero, backdrop, desk character)
  components/ui/          Reveal, Tilt, Section, SplitText, Stagger, Pagination
  hooks/                  scroll, pointer, parallax, device tier, reduced motion
  i18n/LanguageProvider   localizes the content bundle for the active language
  styles/globals.css      ALL design tokens, light/dark, RTL, the .admin palette
...
src/backend/
  ...
  auth/index.js           sessions, bcrypt, requireAdmin (Node runtime)
  auth/session-token.js   JWT verify only — Edge-safe, used by proxy.js
  media/cloudinary.js     upload + credential parsing + error explanations
  admin-ui/               Fields, Forms, AdminNav, Charts, SortableList,
                          LoginForm, PasswordForm, PageHeader
  seed-data/              legacy projects.js + translations.js, used ONLY by
                          `npm run db:seed` to populate a fresh database.
                          They look unused — they are not. Do not delete them
                          without also rewriting scripts/seed.mjs.

src/shared/
  config.js               LANGUAGES, DEFAULT_LANG, LANG_CODES, NAV_IDS
  localize.js             isI18n / pick / localize — { en, fa, ps } → string
  icons/                  BrandIcons.jsx + name→component maps
```

### The import rules that keep the split honest

- `@frontend/…` never imports `@backend/…`. The single deliberate exception is
  `Contact.jsx` importing the `sendMessage` server action — that is this app's
  version of calling an API endpoint. Wanting a second exception is a sign the
  thing belongs in `@shared/`.
- `@backend/…` never imports `@frontend/…`. Not once.
- `@shared/…` imports neither. It stays small on purpose.
- Route files in `src/app/` may import from all three — that is their job.

Run this to check the rules still hold before you commit:

```bash
grep -rn "@backend/" src/frontend/   # expect only Contact.jsx
grep -rn "@frontend/" src/backend/   # expect nothing
grep -rn "@frontend/\|@backend/" src/shared/   # expect nothing
```
...
**Reordering.** `src/backend/admin-ui/SortableList.jsx` gives every collection list
